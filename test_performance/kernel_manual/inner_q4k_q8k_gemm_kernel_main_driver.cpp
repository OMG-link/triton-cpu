...
int main(int argc, char **argv) {
    ...
    // 2) Q4 weights
    block_q4_K *vx = static_cast<block_q4_K *>(calloc(n * nb, sizeof(block_q4_K)));

    // 3) Q8 activations
    block_q8_K *vy = static_cast<block_q8_K *>(calloc(m * nb, sizeof(block_q8_K)));
    ...
    while(T--){
        for(size_t i = 0; i < m; i++) { // M vy 激活
            for(size_t j = 0; j < n; j++) { // N vx 
                float result = 0;
                ggml_vec_dot_q4_K_q8_K(k, &result, n, vx + j * nb, 1, vy + i * nb, 1, 1);
                *(s + i * n + j) = result;
            }
        }
    }
