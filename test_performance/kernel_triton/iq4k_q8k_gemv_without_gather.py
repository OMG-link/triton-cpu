    ...
    for k_block in range(0, K//QK_K):
        ...
        for k_sub_block in range(0, NUM_SUB_BLOCKS):
            ...
            for k in range(QK_4_K_SUB_BLOCK_DATA_SIZE):
                iq4k_data_block_ptr = tl.advance(iq4k_block_ptr, offsets=(0, 0, k_block, k_sub_block, k, 0)) #[NR]@uint8
                iq4k_data_packed = tl.load(iq4k_data_block_ptr).reshape((NR,))  # uint8 * NR, 每个 uint8 包含两个 iq4k 数据 [NR]@uint8 

                iq4k_data_1 = (iq4k_data_packed & 0x0F).reshape(NR).cast(tl.int8) # uint4, [16, NR] 
                iq4k_data_2 = (iq4k_data_packed >> 4).reshape(NR).cast(tl.int8)  # uint4, [16, NR] 
                ...
                sum1 += q8k_data.cast(tl.int16) * iq4k_data_1.cast(tl.int16) 

                q8k_data_ptr = tl.advance(q8k_block_ptr, offsets=(0, k_block, k_sub_block, k + QK_4_K_SUB_BLOCK_DATA_SIZE, 0))
                q8k_data = tl.reshape(tl.load(q8k_data_ptr), (1,)) # [1]@int8，标量会自动广播
                sum2 += q8k_data.cast(tl.int16) * iq4k_data_2.cast(tl.int16)
