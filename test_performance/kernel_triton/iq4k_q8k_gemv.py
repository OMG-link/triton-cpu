...
def iq4k_q8k_gemv_kernel(
    ...
    # q8 向量按超块 & 子块划分: [K//QK_K, NUM_SUB_BLOCKS, QK_K_SUB_BLOCK_SIZE]
    q8k_block_ptr = tl.make_block_ptr(
        base=q8k_vector_ptr,
        shape=(K // QK_K, QK_K // QK_K_SUB_BLOCK_SIZE, QK_K_SUB_BLOCK_SIZE, 1),
    ...
    # IQ4K 数据布局: [N//NR, K//QK_K, NUM_SUB_BLOCKS, QK_4_K_DATA_SIZE, NR]
    ...
    iq4k_scale_l_block_ptr = tl.make_block_ptr(
        ...
        shape=(N//NR, K//QK_K, NUM_SUB_BLOCKS * 2 // 2, NR),
        strides=(K * NR * NUM_SUB_BLOCKS // QK_K, NUM_SUB_BLOCKS * NR, NR, 1),
        offsets=(pid_n, 0, 0, 0),
    ...
    for k_block in range(0, K//QK_K):
        ...
        # 每个 block 一块
        extra_data_ptr = tl.advance(iq4k_extra_block_ptr, offsets=(0, k_block, 0)) # [NR]@int16
        iq4k_extra_data = tl.load(extra_data_ptr) # uint16 * NR

        
        q8k_d_data_ptr = tl.advance(q8k_d_block_ptr, offsets=(k_block,))
        iq4k_d_data_ptr = tl.advance(iq4k_d_block_ptr, offsets=(0, k_block, 0))
        ...
        for k_sub_block in range(0, NUM_SUB_BLOCKS):
            # scale 
            iq4k_scale_l_data_ptr = tl.advance(iq4k_scale_l_block_ptr, offsets=(0, k_block, k_sub_block, 0)) 
            iq4k_scale_l_data = tl.load(iq4k_scale_l_data_ptr)  # uint8 * NR，包含两个 uint4 

            iq4k_scale_h_data_ptr = tl.advance(iq4k_scale_h_block_ptr, offsets=(0, k_block, k_sub_block // 2, 0)) # 每个 subblock 两个 scale_h, uint8 存储 4 个 uint2
            ...
            iq4k_data_block_ptr = tl.advance(iq4k_block_ptr, offsets=(0, k_block, k_sub_block, 0, 0))
            ...
            for k in range(QK_4_K_SUB_BLOCK_DATA_SIZE):
                ...
                q8k_data_ptr = tl.advance(q8k_block_ptr, offsets=(k_block, k_sub_block, k, 0))
                ...
                q8k_data_ptr = tl.advance(q8k_block_ptr, offsets=(k_block, k_sub_block, k + QK_4_K_SUB_BLOCK_DATA_SIZE, 0))
                q8k_data = tl.reshape(tl.load(q8k_data_ptr), (1,)) # [1]@int8，标量会自动广播 
    ...
    acc = acc.reshape((1, NR))
    output_ptr = tl.advance(output_block_ptr, offsets=(0, 0)) 
...
if __name__ == "__main__":
    ...
    if args.simple_test:
        ...
        # 修复: q8k_vector 应该是 4D 以匹配 kernel
