...
def iq4k_q8k_matmul_kernel(
    ...
    value_table = tl.arange(0, 32).cast(tl.int8).reshape(1, NR).broadcast_to((16,NR))
    ...
    for k_block in range(N_block):
        ...
        for sub_block in range(NUM_SUB_BLOCKS):
            ...
            iq4k_data_1 = (iq4k_data_1 & (tlb_start_idx_1 << 4)).reshape(16, NR).cast(tl.int8) 
            iq4k_data_2 = (iq4k_data_packed >> 4).reshape(16, NR)  # uint4, [16, NR] 
            iq4k_data_2 = (iq4k_data_2 & (tlb_start_idx_2 << 4)).reshape(16, NR).cast(tl.int8) 

            # 按行维度 (axis=1) gather：iq4k_data_1 / iq4k_data_2 的取值范围 0..15 

            # iq4k_data_k0_15_dequant_data = tl.gather(src=value_table, index=iq4k_data_1, axis=1)  # (16, NR) 
            ...
            sum1 = tl.dot(q8k_data_k0_15.T, iq4k_data_1, out_dtype = tl.int16)  # int16, shape (12, 32)
            ...
            sum2 = tl.dot(q8k_data_k16_31.T, iq4k_data_2, out_dtype = tl.int16)  # int16, shape (MR, NR)
