...
def iq4k_q8k_gemv_kernel(
    ...
    value_table = tl.arange(0, 32).cast(tl.int8).reshape(32)
    ...
    for k_block in range(0, K//QK_K):
        ...
        for k_sub_block in range(0, NUM_SUB_BLOCKS):
            ...
            tlb_start_idx1 = (iq4k_extra_data >> (2 * k_sub_block)).cast(tl.uint8) & 0x01  # [NR]@uint8
            tlb_start_idx2 = (iq4k_extra_data >> (2 * k_sub_block)).cast(tl.uint8) & 0x02  # [NR]@uint8
            ...
            for k in range(QK_4_K_SUB_BLOCK_DATA_SIZE):
                iq4k_data_block_ptr = tl.advance(iq4k_block_ptr, offsets=(0, 0, k_block, k_sub_block, k, 0)) #[NR]@int8
                # load/gather 配比 1 : 2
                # 如果 走 load：在原来基础 + lux index load 访存查表
                # load + vrgather：load 的延迟 掩盖 vrgather 寄存器查表时间，访存受限，应该可以比较容易跟原来一样的带宽利用率
                iq4k_data_packed = tl.reshape(tl.load(iq4k_data_block_ptr), (NR,))  # [NR]@uint8 每个 uint8 包含两个 iq4k 数据

                iq4k_data_1 = (iq4k_data_packed & 0x0F) #  [NR]@uint4
                iq4k_data_2 = (iq4k_data_packed >>   4) #  [NR]@uint4
                iq4k_data_1 = (iq4k_data_1 | (tlb_start_idx1 << 4)).reshape(NR) # [NR]@uint5
                iq4k_data_2 = (iq4k_data_2 | (tlb_start_idx2 << 4)).reshape(NR) # [NR]@uint5

                # tl.static_print(f"BLOCK_SIZE={NR}")
                
                iq4k_data_dequant1 = tl.gather(value_table, iq4k_data_1, axis = 0)
                iq4k_data_dequant2 = tl.gather(value_table, iq4k_data_2, axis = 0)
                ...
                sum1 += q8k_data.cast(tl.int16) * iq4k_data_dequant1.cast(tl.int16) 
                ...
                q8k_data = tl.reshape(tl.load(q8k_data_ptr), (1,)) # [1]@int8，标量会自动广播 
                sum2 += q8k_data.cast(tl.int16) * iq4k_data_dequant2.cast(tl.int16) 
