...
def q4k_q8k_matmul_kernel(
    ...
    for i_supb in range(0, tl.cdiv(K, QK_K)):
        ...
        # 超块边界 不需要 spill sum_block， sum_block 0 初始化, 后续 summin 需要spill 此变量
        sum_row = tl.cast(sum_block, tl.float32) * tl.dot(a_d.T, b_d, out_dtype=tl.float32) # [12, 32] @ float32 

        a_bsums = tl.load(a_bsums_ptr).reshape((QK_K//QK_SB_K, MR)) #[8, 12]@int16
        b_mins = tl.cast(tl.load(b_mins_ptr).reshape((QK_K//QK_SB_K, NR)), tl.int16) #[8, 32]@int8
        b_dmin = tl.cast(tl.load(b_dmin_ptr).reshape((1, NR)), tl.float32) 
        # submin = tl.cast(tl.dot(a_bsums.T, b_mins, out_dtype=tl.int32) ，超块边界 [12, 32]@int32
