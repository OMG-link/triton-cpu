...
def q4k_q8k_matmul_kernel(
    ...
    for i_supb in range(0, tl.cdiv(K, QK_K)):
        ...
        for i_subb in range(0, tl.cdiv(QK_K, QK_SB_K)):
            ...
            a_bsums_ptr =  tl.advance(a_bsums_ptr_start, (0, i_supb, i_subb, 0))
            b_mins_ptr = tl.advance(b_mins_ptr_start, (0, i_supb, i_subb, 0))
            a_bsums = tl.load(a_bsums_ptr).reshape((1, MR))                  #[1, 12]@int16 
            b_mins = tl.cast(tl.load(b_mins_ptr).reshape((1, NR)), tl.int16) #[1, 32]@int8 
            submin += tl.dot(a_bsums.T, b_mins, out_dtype=tl.int32)          #超块边界 [12, 32]@int32 
        ...
        # 为了存储 c_output, sum_min_row 也需要 spill
        sum_min_row = tl.dot(a_d.T, b_dmin, out_dtype=tl.float32) * tl.cast(submin, tl.float32) 
        # spill c_output
        c_output += (sum_row - sum_min_row)
        
        ## sum_row sum_min_row 超块迭代间只有 c_output 存活， spill 数量减少

    
    # compute final result
    c_ptr = c_ptr_start 
    tl.store(c_ptr, c_output) 
