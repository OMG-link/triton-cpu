...
def bench_iq4k_q8k_case(M, K, N, rep_ms=200, warmup_ms=50, num_threads=None):
    """对 iq4k_q8k kernel 进行单个形状性能测试, 返回时间"""
    iq4k_matrix, iq4k_d, iq4k_extra, iq4k_scale_l, iq4k_scale_h, q8k_matrix, q8k_d, output = prepare_random_iq4k_q8k_inputs(M, K, N)
    grid = (M//MR, N//NR)
    def fn():
        iq4k_q8k_matmul_kernel[grid](
            iq4k_matrix, iq4k_d, iq4k_extra, iq4k_scale_l, iq4k_scale_h,
            q8k_matrix, q8k_d, output, M, N, K, num_threads=num_threads
        )
    # 简易基准测试
    import time
...
def get_kernel_info():
    ...
    return {
        'name': 'iq4k_q8k',
        'bench_fn': bench_iq4k_q8k_case,
        'constraints': {'M': 12, 'N': 32, 'K': 256},
        'compute_dtype': 'int8',  # iq4 x int8, 有效计算宽度 int4 
        'dtype_width': 8,  # bits 
    }
