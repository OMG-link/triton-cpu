...
# float32: [8, 8] * [8, 32] = [8, 32]
# other: [32, 64/type_size_in_bytes] * [64/type_size_in_bytes, 32] = [32, 32]
BLOCK_SIZE_M = 16 if DTYPE == "float32" else 32
BLOCK_SIZE_N = 8
BLOCK_SIZE_K = 64 if DTYPE == "float32" else 64 // in_dtype.itemsize
GROUP_SIZE_M = 32 # 这个值作用？
...
def block_transpose_combined_kernel(in_a, out_a, in_b, out_b, M, N, K, BLOCK_SIZE_M: tl.constexpr,
    ...
    tl.static_assert(BLOCKED_A or not TRANSPOSED_BLOCK_A) # 只有当矩阵 A 启用了分块布局，才能对其进行块转置
    tl.static_assert(BLOCKED_B or not TRANSPOSED_B) 
    pid = tl.program_id(axis=0)
    # 分别计算出整个矩阵在 M 维度（行）和 N 维度（列）上总共被分成了多少个块
    num_pid_m = tl.cdiv(M, BLOCK_SIZE_M) # 除法向上取整，有几块 
    num_pid_n = tl.cdiv(N, BLOCK_SIZE_N) # n 有几块 
    num_pid_in_group = GROUP_SIZE_M * num_pid_n # 定义了一个组负责处理多少“行”的块，
                                                # 这种分组策略可以提高数据局部性（Data Locality）
                                                # 让同一个 CPU 核心组（或 GPU 上的 SM）集中处理内存中相邻的数据，从而提升缓存命中率
    group_id = pid // num_pid_in_group # 计算当前实例属于哪个组 
    first_pid_m = group_id * GROUP_SIZE_M # 起始行块号
    group_size_m = min(num_pid_m - first_pid_m, GROUP_SIZE_M) # 实际行数 
    # 它实际上是在工作组内部实现了一个**列主序（Column-Major）**的布局
    # 也就是说，随着局部 ID 的增加，程序实例会优先填满一列，然后再移动到下一列
    # 例如，如果 group_size_m 是 4，那么：
    # 局部 ID 0-3 会被分配到第 0 列（0//4=0, 1//4=0, ...）
    # 局部 ID 4-7 会被分配到第 1 列（4//4=1, 5//4=1, ...）
    in_block_m = first_pid_m + (pid % group_size_m) # 组的行号 + 在组内的行号 
    in_block_n = (pid % num_pid_in_group) // group_size_m # 列号 
    ...
    if BLOCKED_A:
        ...
        # 所有 N // BLOCK_SIZE_N 个程序实例作为一个小组，
        # 每个实例处理 K 维度上的一个“条带”，它们合在一起，
        # 就能完整、无重复、无遗漏地覆盖矩阵 A 在 K 维度上的所有数据块 
        # 线程一共就 (M // BLOCK_SIZE_M) * (N // BLOCK_SIZE_N) 个
        # k 维度需要重新映射
        for in_block_k in tl.range(in_block_n, A_OUT_BLOCKS_K, N // BLOCK_SIZE_N):
            ...
            if TRANSPOSED_BLOCK_A:
                val = val.T # [BLOCK_SIZE_K, BLOCK_SIZE_M]
...
# LINE_VALS = [
#     "triton-cpu-bb-tb-pb-float32",           # Triton CPU: 分块+转置+打包
#     "triton-cpu-bb-tb-pb-prepack-float32",   # Triton CPU: 分块+转置+打包+预打包
#     "triton-cpu-bb-tb-float32",              # Triton CPU: 分块+转置
#     "triton-cpu-bb-tb-prepack-float32",      # Triton CPU: 分块+转置+预打包
#     "triton-cpu-float32",                    # Triton CPU: 标准布局
#     "triton-cpu-prepack-float32",            # Triton CPU: 标准布局+预打包
#     "torch-cpu-native-float32"               # PyTorch 原生实现
# ]
# 这7个配置代表了：
# 6种 Triton CPU 变体：测试不同的内存布局和数据预处理策略 
# 1种 PyTorch 基准：作为性能对比的参考线 
...
# 被 perf_report 装饰的函数会被多次调用以收集基准数据
# 实现画图的功能 的一个包装器
...
def benchmark(M, N, K, provider):
    ...
    # 根据 Triton CPU dot 算子的一些实现方法，决定数据布局方式
    ...
    if backend == 'torch-cpu-native':
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            ...
            rep=1000)
    perf = lambda ms: 2 * M * N * K * 1e-9 / (ms * 1e-3) # 计算性能 GFLOPS 
...
benchmark.run(print_data=True, show_plots=False, save_path='./results/')
